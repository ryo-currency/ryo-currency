...
Core tests take longer than any other Ryo tests, due to the high amount of computational work involved in validating core components.
...
To run only Ryo's core tests (after building):
...
To run only Ryo's crypto tests (after building):
...
To run only Ryo's hash tests (after building):
...
To run only Ryo's performance tests (after building):
...
To run only Ryo's unit tests (after building):
