...
Configuration::Configuration(const Configuration &c) : m_level(c.m_level),
...
Configuration::Configuration(Level level, ConfigurationType configurationType, const std::string &value) : m_level(level),
...
Configuration::Predicate::Predicate(Level level, ConfigurationType configurationType) : m_level(level),
																						m_configurationType(configurationType)
{
}
...
Configurations::Configurations(void) : m_configurationFile(std::string()),
									   m_isFromFile(false)
{
}

Configurations::Configurations(const std::string &configurationFile, bool useDefaultsForRemaining,
							   Configurations *base) : m_configurationFile(configurationFile),
													   m_isFromFile(false)
{
...
Logger::Logger(const std::string &id, base::LogStreamsReferenceMap *logStreamsReference) : m_id(id),
...
Logger::Logger(const std::string &id, const Configurations &configurations,
			   base::LogStreamsReferenceMap *logStreamsReference) : m_id(id),
																	m_typedConfigurations(nullptr),
																	m_parentApplicationName(std::string()),
																	m_isConfigured(false),
																	m_logStreamsReference(logStreamsReference)
	...
	for(Configurations::const_iterator it = configurations->begin(); it != configurations->end(); ++it)
	{
		...
		else if(conf->configurationType() == ConfigurationType::Format)
		{
			setValue(conf->level(), base::LogFormat(conf->level(),
													base::type::string_t(conf->value().begin(), conf->value().end())),
					 &m_logFormatMap);
		}
...
unsigned long TypedConfigurations::getULong(std::string confVal)
{
	...
	valid = !confVal.empty() && std::find_if(confVal.begin(), confVal.end(), [](char c) {
...
void Writer::initializeLogger(const std::string &loggerId, bool lookup, bool needLock)
{
	...
	else
	{
		...
		{
			m_proceed = m_level == Level::Verbose ? m_logger->enabled(m_level) : ELPP->vRegistry()->allowed(m_level, loggerId.c_str());
		}
		else
		{
...
PerformanceTracker::PerformanceTracker(const std::string &blockName,
									   ...
									   bool scopedLog, Level level) : m_blockName(blockName), m_timestampUnit(timestampUnit), m_loggerId(loggerId), m_scopedLog(scopedLog),
