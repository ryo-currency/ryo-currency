...
{
  public:
	HitCounter(void) : m_filename(""),
					   m_lineNumber(0),
					   m_hitCounts(0)
	{
	}

	HitCounter(const char *filename, base::type::LineNumber lineNumber) : m_filename(filename),
																		  m_lineNumber(lineNumber),
																		  m_hitCounts(0)
	{
	}

	HitCounter(const HitCounter &hitCounter) : m_filename(hitCounter.m_filename),
...
{
  public:
	LogDispatcher(bool proceed, LogMessage &&logMessage, base::DispatchAction dispatchAction) : m_proceed(proceed),
...
class Writer : base::NoCopy
{
  public:
	Writer(Level level, const char *file, base::type::LineNumber line,
		   ...
		   base::type::VerboseLevel verboseLevel = 0) : m_level(level), m_file(file), m_line(line), m_func(func), m_verboseLevel(verboseLevel),
...
class PerformanceTracker : public base::threading::ThreadSafe, public Loggable
{
  public:
	...
	PerformanceTracker(const PerformanceTracker &t) : m_blockName(t.m_blockName), m_timestampUnit(t.m_timestampUnit), m_loggerId(t.m_loggerId), m_scopedLog(t.m_scopedLog),
  ...
  public:
	...
	class StackTraceEntry
	{
	  public:
		...
		StackTraceEntry(std::size_t index, char *loc) : m_index(index),
														m_location(loc)
		{
		}
		std::size_t m_index;
...
#define CVERBOSE(writer, vlevel, dispatchAction, ...)                              \
	...
	writer(                                                                        \
		el::Level::Verbose, __FILE__, __LINE__, ELPP_FUNC, dispatchAction, vlevel) \
